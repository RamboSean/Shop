...
public class CommonUtils <T> {
	...
	//主键回填的公共方法
	public static int addAndReturnPrimaryKey(String sql, Object...objs) {
		//获取连接对象，再获取执行对象，操作添加
		Connection conn = null;
		PreparedStatement prst = null;
		ResultSet rSet = null;
		try {
			...
			prst.executeUpdate();
			rSet = prst.getGeneratedKeys();  //获取产生的主键
			if(rSet.next()){
				return rSet.getInt(1);  //获取第一个字段的值
			}
			
		} catch (SQLException e) {
			e.printStackTrace();
		}finally{
			DBUtils.closeAll(rSet,prst,conn);
		}
		return 0;
	}
