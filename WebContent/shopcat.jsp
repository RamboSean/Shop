...
<script type="text/javascript">
	function sub(id){
		//根据传入id获取到标签对象
		var countObj = document.getElementById("count_"+id);
		if(countObj.value!=1){
			//前端进行数量减少或增加后，同时在购物车中也要进行更新
			countObj.value = countObj.value-1;
			
			var param = new Object();
			param.id = id;
			param.count = countObj.value;
			//异步操作
			$.get("ShoppingServlet?action=update",param,function(data){
				location.reload();  //刷新
			})
		}
	...
	function add(id){
		var countObj = document.getElementById("count_"+id);
		var stockObj = document.getElementById("stock_"+id);
		if(countObj.value!=stockObj.value){
					...
					<div class="bundle  bundle-last ">
						...
						<!-- 购物车 -->
							<ul class="item-content clearfix">
								...
								<li class="td td-amount">
									<div class="amount-wrapper ">
										<div class="item-amount ">
											<div class="sl">
												<input class="min am-btn" name="" onclick="sub(${domain.id})" type="button" value="-" />
											 <input class="text_box" id="count_${domain.id}" type="text" value="${domain.count}" style="width: 30px; text-align: center;" /> 
												<input class="add am-btn" name="" onclick="add(${domain.id})" type="button" value="+"  />
											 	<input type="hidden" id="stock_${domain.id}" value="${domain.goods_stock}" />
											</div>
										</div>
									</div>
								</li>
								<li class="td td-sum">
									<div class="td-inner">
										<em tabindex="0" class="J_ItemSums number" ><fmt:formatNumber value="${domain.count*domain.goods_price}" pattern="0.00"></fmt:formatNumber></em>
									</div>
								</li>
								<li class="td td-op">
									<div class="td-inner">
		...
		<div class="float-bar-wrapper">
			<div class="float-bar-right">
				...
				<div class="price-sum">
					...
					<strong class="price">
					¥<em id="J_Total"></em>
					<fmt:formatNumber value="${shoppingCart.price}" pattern="0.00"></fmt:formatNumber>
